...
class Questions:
    ...
    def get_wrong_answers(self, cca2):
        ...
        data = self._db.execute("SELECT Cities.name FROM Cities, Countries WHERE Cities.cca2=? AND Cities.cca2=Countries.cca2 AND Countries.capital<>Cities.name ORDER BY RANDOM() LIMIT 3", [cca2]).fetchall()  # pylint: disable=line-too-long
        answer_count = len(data)

        if answer_count < 3:
            amount = 3 - answer_count
            extra_answers = self._db.execute("SELECT Cities.name FROM Cities, Countries WHERE Countries.cca2=Cities.cca2 AND Countries.capital<>Cities.name ORDER BY RANDOM() LIMIT ?", [amount]).fetchall()  # pylint: disable=line-too-long
